    ...
    <main className="bg-[#25272B] h-full max-md:p-0 px-52 text-white">
      ...
      <div className="flex flex-col mt-40 justify-end max-md:justify-center items-center gap-x-6 p-2">
        {!start ? (
          <>
            <Image
              className="min-w-12 h-12 max-sm:w-8 max-sm:h-8"
              src={"/Subtract.svg"}
              height={50}
              width={50}
              alt="bot-logo"
            />
            <span className="py-12 max-md:text-xs min-w-[150px] text-center">
              Your daily AI assisstant
            </span>
            <div className="flex gap-x-6 max-md:w-[80vw] max-sm:hidden overflow-x-scroll p-2 max-lg:border max-lg:border-zinc-700 max-lg:scrollbar-hide">
              <Chatcard
                icon="/vector.svg"
                txt="Create a crossword puzzle for me"
              />
              <Chatcard
                icon="/vector (1).svg"
                txt="Plan a budget for my vacation"
              />
              <Chatcard
                icon="/vector (2).svg"
                txt="Ethical implications of AI"
              />
              <Chatcard
                icon="/vector (3).svg"
                txt="Calender for the whole month"
              />
            </div>
          </>
        ) : (
          <ul className=" max-md:w-[90%] max-md:relative max-md:bottom-30 w-[792px] h-[586px] flex flex-col gap-y-8 relative bottom-10 overflow-y-scroll">
            {messages.map((message, index) =>
              message.isLoading ? (
                ...
                message.text !== "" && (
                  <li
                    className={` w-fit ${message.type === "bot" ? "rounded-2xl p-6 bg-[#3A3C40]" : "rounded-xl py-5 px-4 border border-[#3A3C40] ml-auto"}`}
                    key={index}
                  >
                    {message.text}
                  </li>
        ...
        <div className="absolute bottom-16 flex">
          ...
          <button
            ...
            type="button"
            onClick={sendMsg}
            className="p-3.5 bg-[#3B3D40] ml-0.5 rounded-br-xl rounded-tr-xl"
          >
            <Image src="/send.svg" alt="send" height={13} width={14} />
          </button>
        </div>
      </div>
    </main>
